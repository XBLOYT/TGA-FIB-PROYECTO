...
__global__ void Equalize(unsigned int N, unsigned char *image, int *minmaxArray, unsigned int *prob){
	...
	while (i < N){
		...
		unsigned int index = (colorR << 8) | (colorG << 4) | (colorB); 
		unsigned int color = (prob[index] * minmaxArray[2] + PREC * minmaxArray[0])/PREC;
